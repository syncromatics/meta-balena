...
python resin_boot_sanity_handler() {
  ...
  if kernel_file in d.getVar('RESIN_BOOT_PARTITION_FILES', True):
    bb.fatal("ResinOS from v2.14 onwards only supports having the kernel in the root partition in rootfs/boot/KERNEL_IMAGETYPE. Please remove it from RESIN_BOOT_PARTITION_FILES.")
}
